...
export default {
  addNote: {
    ...
    async resolve(source, {note}, {authToken}) {
      ...
      const newNote = await r.table('newsItems').insert(note, {returnChanges: true})
      if (newNote.errors) {
  ...
  updateNote: {
    ...
    async resolve(source, {note}, {authToken}) {
      ...
      const updatedNote = await r.table('newsItems').get(id).update(updates, {returnChanges: true})
